        ...
        link: function (scope, element, attributes) {
            ...
            if (attributes.multiple) {
              options = {
                ...
                droppable: scope.$eval(attributes.droppable),
                thumbnail: 'small'
              };
            } else {
              options = {
            ...
            options.before_remove = function() {
              scope.$apply(function(){
                $fileUploader.current.clearQueue();
              });
              return true;
            };

            options.before_change = function() {
              scope.$apply(function(){
                $fileUploader.current.clearQueue();
              });
              return true;
            ...
            if($fileUploader.current) {
              $fileUploader.current.reset = function (){
...
app.factory('$fileUploader', [ '$compile', '$rootScope', '$http', '$window', function ($compile, $rootScope, $http, $window) {
    ...
    return {
        create: function (params) {
            this.current = new Uploader(params);
            return this.current;
        },
